...
const string strMessageMagic = "BootyCoin Signed Message:\n";
...
{
    // BootyCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 69000 * COIN;

    // Subsidy is cut in half every 600000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 500000); // BootyCoin: 500k blocks in ~3.3 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 6.9 * 24 * 60 * 60; // BootyCoin: 6.9 days
static const int64 nTargetSpacing = 203; // BootyCoin: 3:23 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BootyCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BootyCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // BootyCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].nValue = 69000 * COIN;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // BootyCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BootyCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BootyCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BootyCoinMiner(CWallet *pwallet)
{
    printf("BootyCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bootycoin-miner");
    ...
    try { loop {
        ...
        printf("Running BootyCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BootyCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BootyCoinMiner, pwallet));
}
